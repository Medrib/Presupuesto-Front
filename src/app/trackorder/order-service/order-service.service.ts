...
export class OrderService {
  ...
  exportAsExcelFile(filters: Filters, sorting: Sorting, pagination: Pagination, search: boolean): Observable<[Gastos]> {
    ...
    return this.http.get<[Gastos]>('https://localhost:7026/gastos/search', { params });
  }
  ...
  getDataFromServer(filters: Filters, sorting: Sorting, pagination: Pagination): Observable<{ data: Gastos[], totalItems: number }> {
    ...
    return this.http.get<{ data: Gastos[], totalItems: number }>('https://localhost:7026/gastos/search', { params });
  }
  ...
  getCountOrders(filters: Filters): Observable<{ data: number }> {
    ...
    return this.http.get<{ data: number }>('https://localhost:7026/gastos/orderCount', { params });
  }

  sendDataToServer(nuevoGasto: AgregarGastoRequest): Observable<{ data: string }> {
    return this.http.post<{ data: string }>('https://localhost:7026/gastos/agregarGasto', nuevoGasto);
  }

  postIngreso(nuevoIngreso: AgregarIngresoRequest): Observable<{ data: string }> {
    return this.http.post<{ data: string }>('https://localhost:7026/gastos/agregarIngreso', nuevoIngreso);
  }

  insertCategory(categoria: AgregarCategoriaRequest): Observable<{ data: string }> {
    return this.http.post<{ data: string }>('https://localhost:7026/gastos/agregarCategoria', categoria);
  }

  getCategories(): Observable<Categoria[]> {
    return this.http.get<Categoria[]>('https://localhost:7026/gastos/getCategory');
  }
}
